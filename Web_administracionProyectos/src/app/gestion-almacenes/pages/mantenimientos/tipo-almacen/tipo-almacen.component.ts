...
import { AreasService } from '../../../../services/gestion-almacenes/mantenimientos/areas.service';
 declare var $:any;
...
export class TipoAlmacenComponent implements OnInit {
  ...
  constructor(private alertasService : AlertasService, private spinner: NgxSpinnerService, private loginService: LoginService, private funcionGlobalServices : FuncionesglobalesService, 
    private areasService : AreasService, private combosService : CombosService ) {         
 ...
 inicializarFormulario(){ 
    this.formParams= new FormGroup({
        ...
        nombre_TipoAlmacen: new FormControl(''), 
 ...
 mostrarInformacion(){ 
    this.spinner.show();
    this.areasService.get_mostrar_informacion( this.formParamsFiltro.value.estado  )
 ...
 async saveUpdate(){ 
  ...
  if (this.formParams.value.nombre_TipoAlmacen == '' || this.formParams.value.nombre_TipoAlmacen == null) {
  ...
  if ( this.flag_modoEdicion==false) { //// nuevo  
     ...
     Swal.showLoading(); 
    this.areasService.set_save_TiposAlmacen (this.formParams.value)
      .subscribe((res:any)=>{  
        Swal.close();    
        if (res.ok ==true) {     
     ...
     Swal.showLoading();    
    this.areasService.set_edit_TiposAlmacen (this.formParams.value , this.formParams.value.id_TipoAlmacen)
      .subscribe((res:any)=>{  
        Swal.close();    
        if (res.ok ==true) {     
          this.flag_modoEdicion = true;
 ...
 editar({id_TipoAlmacen, nombre_TipoAlmacen, estado   }){
   this.flag_modoEdicion=true;
    this.formParams= new FormGroup({
      id_TipoAlmacen: new FormControl(id_TipoAlmacen), 
      nombre_TipoAlmacen: new FormControl(nombre_TipoAlmacen), 
 ...
 anular(objBD:any){
   ...
   .then((result)=>{
     if(result.value){
      ...
      Swal.showLoading();    
     this.areasService.set_anular_TiposAlmacen(objBD.id_TipoAlmacen).subscribe((res:RespuestaServer)=>{
         Swal.close();        
         if (res.ok ==true) { 
           
           for (const user of this.TiposAlmacen ) {
             if (user.id_TipoAlmacen == objBD.id_TipoAlmacen ) {
                 user.estado = 2;
                 break;
             }
           }

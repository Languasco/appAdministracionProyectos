...
export class LoginService {
  ...
  public guardarSesion(data:any){
    ...
    localStorage.setItem('data_GestionProyectos_user', JSON.stringify(data));
  ...
  leerSesion(){    
   // si es que existe una  variable creada en el local storage, leemos su valor
    if (localStorage.getItem('data_GestionProyectos_user')) { 
      this.usuarioLogeado =true;
      this.dataLogeado =  JSON.parse(localStorage.getItem("data_GestionProyectos_user")|| '[]');
    }else{  
  ...
  logOut(){
    this.usuarioLogeado= false;
    localStorage.removeItem('data_GestionProyectos_user');

...
export class CargoPersonalService {
  ...
  set_save_cargos(objMantenimiento:any):any{
    return this.http.post(this.URL + 'tblCargo_Personal', JSON.stringify(objMantenimiento), httpOptions);
  }

  set_edit_cargos(objMantenimiento:any, id :number):any{
    return this.http.put(this.URL + 'tblCargo_Personal/' + id , JSON.stringify(objMantenimiento), httpOptions);
  }

  set_anular_cargos(id : number):any{ 
    let parametros = new HttpParams();
  ...
  get_verificar_nombre(nombre:string){
    let parametros = new HttpParams();
    parametros = parametros.append('opcion', '3');
    parametros = parametros.append('filtro', nombre);
